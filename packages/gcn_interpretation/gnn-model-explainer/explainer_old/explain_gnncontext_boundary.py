...
class ExplainerPGExplainerBoundary(explain.Explainer):
    def __init__(
        self,
        model,
        adj,
            ...
            with torch.no_grad():
                ...
                if self.args.noise:
                    ...
                    for n_iter in range(noise_iters):
                        noise_orig_adj, added_edges, removed_edges = noise_utils.addNoiseToGraph(sub_adj[0],
                                                                                                 sub_feat[0], h_nodes,
                                                                                                 sub_nodes,
                                                                                                 self.args.noise_percent)
                        ...
                        emb_noise = self.model.getEmbeddings(x, noise_orig_adj_t.unsqueeze(0),
                                                             [sub_nodes.cpu().numpy()])

                        print("debug sum: ", torch.sum(emb).item(), torch.sum(emb_noise2).item(),
                              torch.sum(emb_noise).item(), emb.shape, emb_noise2.shape)
                        pred_n, masked_adj_n, _, _, _ = explainer(
                            (x[0], emb_noise[0], noise_orig_adj_t, tmp, label, sub_nodes), training=False)
                        ...
                        pred_noise_adj, _ = self.model(x.cuda(), noise_orig_adj_t.unsqueeze(0),
                                                       batch_num_nodes=[sub_nodes.cpu().numpy()])
                    ...
                    if len(masked_adjs_noise) > 0:
                        noise_diff = noise_utils.compare_adjs(masked_adj, masked_adjs_noise, orig_adj, noise_orig_adjs,
                                                              h_nodes=None, use_comb_mask=use_comb_mask)
    ...
    def explain_graphs(self, args, graph_indices):
        ...
        explainer = ExplainModule(
            ...
            device=self.device,
            args = self.args,
            orig_input_dim=self.feat.shape[2],
            orig_classes=self.pred.shape[2]
        )
        ...
        if self.args.bmname == "synthetic" or self.args.bmname == "old_synthetic":
            ...
            size = 1427
        elif self.args.bmname == "tox21":
            size = 5066
        elif self.args.bmname == "BA_2Motifs":
            size = 700
        elif self.args.bmname == "Graph-SST2":
            size = 46332
        else:
            assert (False)
        if args.percent_data > 0.:  # in this case, filtering was not done based on label so do it now
            size = len(graph_indices)
            if args.multigraph_class >= 0:
                preds_data = np.argmax(self.pred[0, :size, :], axis=1)
                new_graph_indices = []
                for i_l, l in enumerate(preds_data):
                    if l == args.multigraph_class:
                        new_graph_indices.append(i_l)
                graph_indices = new_graph_indices
        ...
        myfile.write("\n percent_data: {}, multigraph class: {}".format(self.args.percent_data, self.args.multigraph_class))
        ...
        for epoch in range(self.args.start_epoch, self.args.num_epochs):
            ...
            for graph_idx in graph_indices:
                ...
                pred, masked_adj, graph_embedding, inv_embedding, inv_pred = explainer((x[0], emb, adj[0], tmp, label, sub_nodes), training=training)
                if pred is not None:
                    print("debug pred: ", self.pred[0][graph_idx], pred, gt_pred)
...
class ExplainModule(nn.Module):
    ...
    def forward(self,inputs,node_idx=None, training=None):
        ...
        new_embed = self.gnn_explainer.getEmbeddings(x.unsqueeze(0), adj.unsqueeze(0),
                                                     batch_num_nodes=[sub_nodes.cpu().numpy()])
        # print("new embed: ", embed.shape, new_embed.shape)
        embed = new_embed[0]
        ...
        if self.args.max_mask:
            sym_mask = torch.max(sym_mask, dim=1)[0]
            sym_mask = sym_mask.unsqueeze(1).expand(adj.shape[0], adj.shape[1])
            self.mask = sym_mask
            masked_adj = torch.mul(adj, sym_mask)
            masked_adj = torch.mul(masked_adj, sym_mask.T)
            print("Node density: ",
                  (torch.sum(torch.max(masked_adj, dim=1)[0]) / torch.sum(torch.max(adj, dim=1)[0])).item())
            self.adj = adj
        ...
        if self.args.gumbel:
            ...
            if self.args.max_mask:
                ...
                inverse_masked_adj = torch.mul(inverse_masked_adj, inverse_mask.T)
            else:
                inverse_masked_adj = torch.mul(adj, inverse_mask)
            self.inverse_masked_adj = inverse_masked_adj
        ...
        if self.args.inverse_boundary_c > 0.0:
            ...
            if node_idx is not None:
                # node mode
                inv_output, inv_embed = self.model(inv_x, inv_adj, batch_num_nodes=sub_num_nodes_l, new_node_idx=[node_idx])

                inv_res = self.softmax(inv_output[0][node_idx, :])
            else:
                # graph mode
                inv_output, inv_embed = self.model(inv_x, inv_adj, batch_num_nodes=sub_num_nodes_l)
    ...
    def loss(self, pred, pred_label, node_idx=None, graph_embedding=None, boundary_list=None, gt_embedding=None, inv_embedding=None):
        ...
        if self.args.max_mask:
            size_loss = self.args.size_c * torch.sum(torch.max(self.masked_adj, dim=1)[0]) / torch.sum(torch.max(self.adj, dim=1)[0])
            mask = self.masked_adj * 0.99 + 0.005  # maybe a .clone()
            mask_ent = -mask * torch.log(mask) - (1 - mask) * torch.log(1 - mask)
            mask_ent_loss = self.args.ent_c * (torch.sum(mask_ent)/torch.sum(mask))
