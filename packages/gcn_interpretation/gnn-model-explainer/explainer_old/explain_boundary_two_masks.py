...
class ExplainerBoundaryTwoMasks(explain.Explainer):
    def __init__(
        self,
        model,
        adj,
    ...
    def explain_nodes_gnn_stats(self, node_indices, graph_node_indices, args, graph_idx=0, model="exp"):
        ...
        log_file = self.args.prefix + "log_pgeboundary_" + self.args.bmname + ".txt"
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for i, node_idx in enumerate(node_indices):
                ...
                pred, masked_adj, graph_embedding, inv_embedding, inv_pred = explainer(
                    (x[0], emb[0], adj[0], tmp, label, None), node_idx=node_idx_new, training=True)

                masked_adj_2 = explainer.masked_adj_2.cpu().clone().detach().numpy()
                ...
                if self.args.bmname == 'syn3':
                    map_score, h_edges = accuracy_utils.getmAPsyn3(masked_adj, n_adj, labels, nbrs, new_idx, self.adj[0])

                elif self.args.bmname == 'syn4':
                    # map_score = getmAPsyn3(masked_adjs[i], n_adj, labels, nbrs, new_idx, self.adj[0])
                    map_score, h_edges = accuracy_utils.getmAPsyn4(masked_adj, n_adj, labels, nbrs, new_idx, self.adj[0])
                elif self.args.bmname == 'syn1':
                    map_score, h_edges = accuracy_utils.getmAPsyn1(masked_adj, n_adj, labels, nbrs, new_idx, self.adj[0])
                elif self.args.bmname == 'syn2':
                    map_score, h_edges = accuracy_utils.getmAPsyn2(masked_adj, n_adj, labels, nbrs, new_idx, self.adj[0])
                else:
                ...
                mask_density_2 = np.sum(masked_adj_2) / np.sum(n_adj)

                assert(np.abs(np.sum(n_adj) - np.sum(sub_adj)) < 0.1)
                avg_mask_density += mask_density
                avg_mask_density_2 += mask_density_2
            ...
            avg_mask_density_2 = avg_mask_density_2 / len(node_indices)
            ...
            print(
                "Average mask density_2: {}".format(avg_mask_density_2)
            )
            myfile.write("\n Average mask density_2: {}".format(avg_mask_density_2))
            ...
            with torch.no_grad():
                ...
                if self.args.noise:
                    ...
                    for n_iter in range(noise_iters):
                        ...
                        noise_orig_adj_t = torch.from_numpy(noise_orig_adj).float()
                        if args.gpu:
                            noise_orig_adj_t = noise_orig_adj_t.cuda()
                            x = x.cuda()
                            adj = adj.cuda()


                        emb_noise = self.model.getEmbeddings(x, noise_orig_adj_t.unsqueeze(0), [sub_nodes.cpu().numpy()])

                        print("debug sum: ",  torch.sum(emb).item(), torch.sum(emb_noise2).item(), torch.sum(emb_noise).item(), emb.shape, emb_noise2.shape)
                        pred_n, masked_adj_n,_, _, _ = explainer((x[0], emb_noise[0], noise_orig_adj_t, tmp, label, sub_nodes), training=False)

                        print("debug: ", graph_idx, pred_label, torch.argmax(pred[0]).item(), pred_n[0])
                        masked_adj_n = masked_adj_n.cpu().detach().numpy() * noise_orig_adj
                        masked_adj_n_sfmx = np.exp(masked_adj_n) / np.sum(np.exp(masked_adj_n))
                        noisy_mAP_s = accuracy_utils.getmAP(masked_adj_n, h_nodes)
                        noise_AUC.addEdges(masked_adj_n, h_nodes, dataset='synthetic')

                        noise_mAP += noisy_mAP_s
                        print("mAP: ", mAP_s, noisy_mAP_s)
                        # exit()


                        pred_noise_adj, _ = self.model(x.cuda(), noise_orig_adj_t.unsqueeze(0), batch_num_nodes=[sub_nodes.cpu().numpy()])
                        if torch.argmax(pred_noise_adj[0]).item() == pred_label:
                        # if torch.argmax(pred[0]).item() == pred_label:
                            masked_adjs_noise.append(masked_adj_n)
                            noise_orig_adjs.append(noise_orig_adj)
                            # masked_adjs_noise.append(masked_adj_n_sfmx)
                    ...
                    if len(masked_adjs_noise) > 0:
                        noise_diff = noise_utils.compare_adjs(masked_adj, masked_adjs_noise, orig_adj, noise_orig_adjs, h_nodes=None, use_comb_mask=use_comb_mask)
                        # noise_diff = noise_utils.compare_adjs(masked_adj_sfmx, masked_adjs_noise, orig_adj, noise_orig_adjs, h_nodes=None, use_comb_mask=use_comb_mask)

                        avg_noise_diff += noise_diff
    ...
    def explain_graphs(self, args, graph_indices):
        ...
        if self.args.gumbel:
            training = False
        if self.args.ent_c_2 < 0.:
            self.args.ent_c_2 = self.args.ent_c
        if self.args.size_c_2 < 0.:
            self.args.size_c_2 = self.args.size_c*0.1
        assert(self.args.intersec_c > 0.)
        ...
        myfile.write("\n intersec cf: {}, size cf_2: {}, ent cf_2 {}".format(self.args.intersec_c, self.args.size_c_2, self.args.ent_c_2))
        myfile.close()
        bloss_prev = None
        ...
        for epoch in range(self.args.start_epoch, self.args.num_epochs):
            ...
            for graph_idx in graph_indices:
                ...
                mask_density_2 = np.sum(masked_adj_2) / np.sum(adj.cpu().numpy())
                avg_mask_density_2 += mask_density_2
                label = self.label[graph_idx]
            ...
            print(
                "Average mask density_2: {}".format(avg_mask_density_2 / rule_acc_count)
            )
            myfile.write("\n Average mask density_2: {}".format(avg_mask_density_2 / rule_acc_count))
...
class ExplainModule(nn.Module):
    ...
    def get_sym_mask(self, node_idx, adjs, values):
        if node_idx is not None:
            sparsemask = torch.sparse.FloatTensor(
                indices=torch.transpose(torch.cat(
                    [torch.unsqueeze(torch.tensor(adjs.row), -1), torch.unsqueeze(torch.tensor(adjs.col), -1)], dim=-1),
                                        0, 1).to(torch.int64).to(self.device),
                values=values,
                size=adjs.shape
            ).to(self.device)
        else:
            sparsemask = torch.sparse.FloatTensor(
                indices=torch.transpose(
                    torch.cat([torch.unsqueeze(self.row, -1), torch.unsqueeze(self.col, -1)], dim=-1), 0, 1).to(
                    torch.int64),
                values=values,
                size=[self.num_nodes, self.num_nodes]
            ).to(self.device)
        sym_mask = sparsemask.coalesce().to_dense().to(
            torch.float32)  # FIXME: again a reorder() is omitted, maybe coalesce
        return sym_mask
    ...
    def forward(self,inputs,node_idx=None, training=None):
        ...
        sym_mask = self.get_sym_mask(node_idx, adjs, values)
        ...
        masked_inp = masked_adj.reshape((-1, 1))

        if node_idx == None:
            h_res = torch.cat((h_res, masked_inp),1)
        else:
            h_res = torch.cat([h_res, masked_inp[adjs.row], masked_inp[adjs.col]], 1)

        for flayer in self.flayers:
            h_res = flayer(h_res)

        m2_values = torch.reshape(h_res, [-1])

        m2_values = self.concrete_sample(m2_values, beta=tmp, training=training)

        sym_mask_2 = self.get_sym_mask(node_idx, adjs, m2_values)
        self.mask_2 = sym_mask_2
        sym_mask_2 = (sym_mask_2 + sym_mask_2.T) / 2

        masked_adj_2 = torch.mul(adj, sym_mask_2)
        self.masked_adj_2 = masked_adj_2
        ...
        adj_2 = torch.unsqueeze(self.masked_adj_2,0).to(torch.float32 )
        ...
        if self.args.boundary_c > 0.0:
            if node_idx is not None:
                # node mode
                output, g_embed = self.model(x, adj_2, batch_num_nodes=sub_num_nodes_l, new_node_idx=[node_idx])

                res = self.softmax(output[0][node_idx, :])
            else:
                # graph mode
                output, g_embed = self.model(x, adj_2, batch_num_nodes=sub_num_nodes_l)
    ...
    def loss(self, pred, pred_label, node_idx=None, graph_embedding=None, boundary_list=None, gt_embedding=None, inv_embedding=None):
        ...
        intersec_loss = self.args.intersec_c*torch.sum(torch.pow(self.masked_adj - (self.masked_adj_2*self.masked_adj), 2))
        ...
        if self.args.size_c > -0.001:
            ...
            size_loss_2 = self.args.size_c_2 * torch.sum(mask_2)
        else:
            assert(False)
        # entropy
        mask = mask *0.99+0.005     # maybe a .clone()
        mask_2 = mask_2 * 0.99 + 0.005

        mask_ent = -mask * torch.log(mask) - (1 - mask) * torch.log(1 - mask)
        mask_ent_2 = -mask_2 * torch.log(mask_2) - (1 - mask_2) * torch.log(1 - mask_2)

        if self.args.ent_c > -0.001:
            mask_ent_loss = self.args.ent_c * torch.mean(mask_ent)
            mask_ent_loss_2 = self.args.ent_c_2 * torch.mean(mask_ent_2)
        ...
        loss = net_boundary_loss + size_loss + mask_ent_loss + size_loss_2 + mask_ent_loss_2 +intersec_loss
        ...
        print("intersec_loss loss: ", intersec_loss.item(), "size_loss_2: ", size_loss_2.item(), "mask ent loss_2: ", mask_ent_loss_2.item())
