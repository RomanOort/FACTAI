...
class ExplainerPGExplainer(explain.Explainer):
    ...
    def explain_nodes_gnn_stats(self, node_indices, graph_node_indices, args, graph_idx=0, model="exp"):
        ...
        myfile.write(
            "\n lr: {}, bound cf: {}, size cf: {}, ent cf {}, lap cf {}, inv cf: {}".format(self.args.lr,
                                                                                            self.args.boundary_c,
                                                                                            self.args.size_c,
                                                                                            self.args.ent_c,
                                                                                            self.args.lap_c,
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for i, node_idx in enumerate(node_indices):
                ...
                if self.args.bmname == 'syn3':
                    ...
                    map_score, _ = accuracy_utils.getmAPNodes(masked_adj, n_adj, labels, nbrs, new_idx)
    ...
    def eval_graphs(self, args, graph_indices, explainer, model):
        ...
        for graph_idx in graph_indices:
            ...
            emb_on_fly = model.getEmbeddings(x.cuda(), adj.cuda(), [sub_nodes.cpu().numpy()])
            if self.args.shuffle_adj:
                emb = emb_on_fly
            ...
            if self.args.noise:
                ...
                for n_iter in range(noise_iters):
                    noise_orig_feat, changed_nodes = noise_utils.addNoiseToGraph2(sub_adj[0], sub_feat[0], h_nodes, sub_nodes, self.args.noise_percent)
                    noise_orig_adj = sub_adj[0]
                    # noise_orig_adj, added_edges, removed_edges = noise_utils.addNoiseToGraph(sub_adj[0], sub_feat[0], h_nodes, sub_nodes, noise_percent)
                    # avg_add_edges += added_edges
                    # avg_removed_edges += removed_edges
                    adj_diff = np.sum(np.abs(sub_adj[0]-noise_orig_adj))
                    feat_diff = np.sum(np.abs(sub_feat[0]-noise_orig_feat))

                    avg_adj_diff += adj_diff
                    avg_feat_diff += feat_diff

                    global_noise_count += 1.0
                    noise_orig_adj_t = torch.from_numpy(noise_orig_adj).float()
                    noise_orig_feat_t = torch.from_numpy(noise_orig_feat).float()

                    if args.gpu:
                        noise_orig_adj_t = noise_orig_adj_t.cuda()
                        noise_orig_feat_t = noise_orig_feat_t.cuda()
                        x = x.cuda()
                        adj = adj.cuda()


                    emb_noise = model.getEmbeddings(noise_orig_feat_t.unsqueeze(0), noise_orig_adj_t.unsqueeze(0), [sub_nodes.cpu().numpy()])

                    print("debug sum: ",  torch.sum(emb).item(), torch.sum(emb_on_fly).item(), torch.sum(emb_noise).item())
                    pred_n, masked_adj_n = explainer((noise_orig_feat_t, emb_noise[0], noise_orig_adj_t, tmp, label, sub_nodes), training=False)
                    ...
                    pred_noise_adj, _ = self.model(noise_orig_feat_t.unsqueeze(0), noise_orig_adj_t.unsqueeze(0), batch_num_nodes=[sub_nodes.cpu().numpy()])
        ...
        print(
            "noise percent: {}".format(self.args.noise_percent)
        )
        myfile.write("\n noise percent: {}".format(self.args.noise_percent))
        ...
        print(
            "avg feat diff: {}".format(avg_feat_diff / global_noise_count)
        )
        print(
        ...
        for epoch in range(self.args.num_epochs):
            ...
            for graph_idx in graph_indices:
                ...
                if self.args.bmname != 'synthetic' and self.args.bmname != 'old_synthetic':
                    ...
                    t1 = 4.99
                tmp = float(t0 * np.power(t1 / t0, epoch /self.args.num_epochs))
                pred, masked_adj = explainer((x[0], emb, adj[0], tmp, label, sub_nodes),training=True)
...
class ExplainModule(nn.Module):
    def __init__(
        ...
        # print(orig_input_dim, self.args.hidden_dim, self.args.output_dim,orig_classes,self.args.num_gc_layers)
    ...
    def forward(self,inputs,node_idx=None, training=None):
        ...
        new_embed = self.gnn_explainer.getEmbeddings(x.unsqueeze(0), adj.unsqueeze(0), batch_num_nodes=[sub_nodes.cpu().numpy()])
