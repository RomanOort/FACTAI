...
class ExplainerRCExplainer(explain.Explainer):
    ...
    def eval_graphs_2(self, args, graph_indices, explainers):
        ...
        noise_handlers = [noise_utils.AdversarialNoiseHandler("RCExplainer", self.model, self, noise_percent=0, adversarial_file=adversarial_file)]
        ...
        for graph_idx in graph_indices:
            with torch.no_grad():
                ...
                pred, masked_adj, _, _, inv_pred = explainers[pred_label]((x[0], emb[0], adj[0], tmp, label, sub_nodes),
                                                             training=False)
                    ...
                    if pred_label == args.multigraph_class:
                        for n_iter in range(noise_iters):
                            for nh in noise_handlers:
                                noise_feat, noise_adj, noise_label = nh.sample(sub_feat[0], sub_adj[0], sub_nodes, graph_idx=idx)
                                emb_noise = self.model.getEmbeddings(noise_feat.unsqueeze(0), noise_adj.unsqueeze(0), [sub_nodes.cpu().numpy()])
                                pred_n, masked_adj_n, _, _, _ = explainers[noise_label]((noise_feat, emb_noise[0], noise_adj, tmp, label, sub_nodes), training=False)
                                masked_adj_n = masked_adj_n.cpu().detach() * noise_adj

                                nh.update(masked_adj, masked_adj_n.cpu().detach().numpy(), sub_adj[0], noise_adj.cpu().detach().numpy(), None, graph_idx, noise_label)
                        idx += 1
        if not args.noise:
    ...
    def explain_graphs(self, args, graph_indices, test_graph_indices=None):
        ...
        if self.args.eval or self.args.exp_path1 != "":
            if self.args.eval and self.args.exp_path1 == "":
                print("no explainer file to load")
                exit()
            else:
                print("loading initial explainer ckpt from : ", self.args.exp_path1)

            for i in range(2):
                if i == self.args.multigraph_class:
                    try:
                        state_dict = torch.load(self.args.exp_path1)
                    except:
                        state_dict = torch.load(self.args.exp_path1 + 'rcexplainer.pth.tar')
                else:
                    try:
                        state_dict = torch.load(self.args.exp_path2)
                    except:
                        state_dict = torch.load(self.args.exp_path2 + 'rcexplainer.pth.tar')
                exp_state_dict = explainers[i].state_dict()
                for name, param in state_dict.items():
                    if name in exp_state_dict and not ("model" in name):
                        exp_state_dict[name].copy_(param)
                explainers[i].load_state_dict(exp_state_dict)



            if self.args.eval:
                self.eval_graphs_2(args, graph_indices, explainers)
                exit()
