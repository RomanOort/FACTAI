...
class ExplainerRCExplainer(explain.Explainer):
    ...
    def eval_nodes(self, node_indices, graph_node_indices, args, graph_idx=0, model="exp"):
        ...
        avg_acc = 0.0
        avg_node = 0.0
        stats = accuracy_utils.Stats("RCExplainer", self, model, graph_mode=False)
    ...
    def eval_graphs_2(self, args, graph_indices, explainer):
        ...
        for graph_idx in tqdm(graph_indices):
            ...
            with torch.no_grad():
                ...
                stats.update(masked_adj, imp_nodes, adj=adj, feat=x, label=label, sub_nodes=sub_nodes)
    ...
    def explain_graphs(self, args, graph_indices, test_graph_indices=None):
        ...
        #     self.adj[graph_idx, rand_order, :] = self.adj[graph_idx,order,:]
        #     self.adj[graph_idx, :, rand_order] = self.adj[graph_idx,:,order]
        log_name = self.args.prefix + f"_seed_{args.seed}_sparsity_" \
                                      f"{self.args.train_data_sparsity}_logdir_RIGHT_HYPERPARAMS"
        log_path = os.path.join(self.args.ckptdir, log_name)
        if os.path.isdir(log_path):
            print("log dir already exists and will be overwritten")
            time.sleep(5)
        ...
        for epoch in tqdm.tqdm(range(self.args.start_epoch, self.args.num_epochs)):
            ...
            if epoch %10 == 0:
                ...
                f_path = self.args.prefix + "explainer_" + self.args.bmname \
                         + f"_seed_{self.args.seed}_sparsity_" \
                           f"{self.args.train_data_sparsity}_RIGHT_HYPERPARMS" \
                           f".pth.tar"
                save_path = os.path.join(log_path, f_path)
                torch.save(explainer.state_dict(), save_path)
                myfile.write("\n ckpt saved at {}".format(save_path))
