        ...
        for epoch in range(self.args.start_epoch, self.args.num_epochs):
            ...
            for graph_idx in graph_indices:
                ...
                loss, bloss_s = explainer.loss(pred, pred_label, graph_embedding=graph_embedding,
                               boundary_list=boundary_list, gt_embedding=gt_embedding,  inv_pred=inv_pred)
                # bloss_ep += bloss_s
                if self.args.bmname != 'synthetic' and self.args.bmname != 'old_synthetic':
                    if ep_count < 200:
                        loss_ep += loss
...
class ExplainModule(nn.Module):
    ...
    def loss(self, pred, pred_label, node_idx=None, graph_embedding=None, boundary_list=None, gt_embedding=None, inv_pred=None, inv_pred_label=None):
        """
        Args:
        ...
        if inv_pred is not None:
            if node_idx is not None:
                # node mode
                inv_pred_label_node = inv_pred_label[node_idx]
                inv_logit = inv_pred[inv_pred_label_node]
            else:
                # graph mode
                inv_pred_reduce = inv_pred[0]
                gt_label_node = self.label
                inv_logit = inv_pred_reduce[gt_label_node]
            # inv_pred_loss = torch.log(inv_logit)
